...
if($_SERVER['REQUEST_METHOD'] == "POST"){
    ...
    //     'email'=>['required','regex:pattern','unique:users,email'],
    //     'phone'=>['required','regex:pattern','unique:users,phone'],
    //     'password'=>['required','regex:pattern','confirmed:password_confirmation'],
    ...
    $validation->setInput($_POST['email'] ?? "")->setInputName('email')->required()->regex('/^([a-zA-Z0-9_\-\.]+)@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.)|(([a-zA-Z0-9\-]+\.)+))([a-zA-Z]{2,4}|[0-9]{1,3})(\]?)$/')->unique();
    $validation->setInput($_POST['phone'] ?? "")->setInputName('phone')->required()->regex('/01[0125][0-9]{8}$/')->unique();
